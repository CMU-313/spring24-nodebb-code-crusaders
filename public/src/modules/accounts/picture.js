...
define('accounts/picture', ['pictureCropper', 'api', 'bootbox', 'alerts'], (pictureCropper, api, bootbox, alerts) => {
    const Picture = {};
        ...
        socket.emit(
            ...
            function (err, pictures) {
                ...
                app.parseAndTranslate(
                    'partials/modals/change_picture_modal',
                    {
                        pictures: pictures,
                        uploaded: uploaded,
                        icon: {
                            text: ajaxify.data['icon:text'],
                            bgColor: ajaxify.data['icon:bgColor'],
                        },
                    ...
                    function (html) {
                        ...
                        function updateImages() {
                            ...
                            if (!ajaxify.data.picture) {
                                modal
                                    .find('.list-group-item .user-icon')
                            ...
                            const radioEl = document.querySelector(
                                `.modal input[type="radio"][value="${ajaxify.data['icon:bgColor']}"]`
                            );
                            if (radioEl) {
                        ...
                        function saveSelection() {
                            ...
                            const iconBgColor =
                                document.querySelector('.modal.picture-switcher input[type="radio"]:checked').value ||
                                'transparent';

                            changeUserPicture(type, iconBgColor)
                                .then(() => {
    ...
    function handleImageUpload(modal) {
        function onUploadComplete(urlOnServer) {
            urlOnServer =
                (!urlOnServer.startsWith('http') ? config.relative_path : '') + urlOnServer + '?' + Date.now();
    ...
    function changeUserPicture(type, bgColor) {
        return api.put(`/users/${ajaxify.data.theirid}/picture`, {
            type,
            bgColor,
        });
    }
