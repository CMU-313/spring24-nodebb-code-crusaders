...
define('forum/post-queue', ['categoryFilter', 'categorySelector', 'api', 'alerts', 'bootbox'], function (
    categoryFilter,
    categorySelector,
    api,
    alerts,
    ...
    PostQueue.init = function () {
        ...
        $('.posts-list').on('click', '[data-action]', async function () {
            ...
            if (
                !['accept', 'reject', 'notify'].includes(action) ||
                (action === 'reject' && !(await confirmReject('[[post-queue:confirm-reject]]')))
            ) {
                return;
            }
                            ...
                            app.parseAndTranslate(
                                ...
                                function (html) {
                                    if ($this.find('.category-text').length) {
                                        $this
                                            .find('.category-text')
                                            .text(html.find('.topic-category .category-text').text());
                                    } else {
    ...
    function handleBulkActions() {
        $('[component="post-queue/bulk-actions"]').on('click', '[data-action]', async function () {
            ...
            if (bulkAction === 'accept-selected' || bulkAction === 'reject-selected') {
                queueEls = queueEls.filter((i, el) => $(el).find('input[type="checkbox"]').is(':checked'));
            }
            const ids = queueEls.map((i, el) => $(el).attr('data-id')).get();
            const showConfirm = bulkAction === 'reject-all' || bulkAction === 'reject-selected';
            if (
                !ids.length ||
                (showConfirm && !(await confirmReject(`[[post-queue:${bulkAction}-confirm, ${ids.length}]]`)))
            ) {
                return;
            }
