...
define('forum/topic/threadTools', [
        ...
        function changeWatching(type, state = 1) {
            ...
            api[method](
                ...
                () => {
                    ...
                    hooks.fire('action:topics.changeWatching', {
                        tid: tid,
                        type: type,
                    });
                },
    ...
    function renderMenu(container) {
        container.on('show.bs.dropdown', '.thread-tools', function () {
            ...
            socket.emit(
                'topics.loadTopicTools',
                { tid: ajaxify.data.tid, cid: ajaxify.data.cid },
                function (err, data) {
                    if (err) {
                        return alerts.error(err);
                    }
    ...
    function topicCommand(method, path, command, onComplete) {
        ...
        const execute = function (ok) {
            if (ok) {
                api[method](`/topics/${tid}${path}`, body).then(onComplete).catch(alerts.error);
            }
        };
    ...
    ThreadTools.setLockedState = function (data) {
        ...
        components
            ...
            .toggleClass('hidden', ajaxify.data.privileges.isAdminOrMod || !data.isLocked || ajaxify.data.deleted);
        ...
        threadEl
            .find('[component="post"][data-uid="' + app.user.uid + '"].deleted [component="post/tools"]')
    ...
    ThreadTools.setDeleteState = function (data) {
        ...
        components
            .get('topic/reply/locked')
            .toggleClass('hidden', ajaxify.data.privileges.isAdminOrMod || !ajaxify.data.locked || data.isDelete);
        threadEl
            .find(
                '[component="post"]:not(.deleted) [component="post/reply"], [component="post"]:not(.deleted) [component="post/quote"]'
            )
    ...
    function setFollowState(state) {
        ...
        translator.translate(titles[state], function (translatedTitle) {
            $('[component="topic/watch"] button').attr('title', translatedTitle).tooltip('fixTitle');
        });
