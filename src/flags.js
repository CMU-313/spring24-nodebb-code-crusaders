...
Flags.getFlagIdsWithFilters = async function ({ filters, uid, query }) {
    ...
    if (orSets.length) {
        let _flagIds = await Promise.all(
            orSets.map(
                async orSet => await db.getSortedSetRevUnion({
                    sets: orSet,
                    start: 0,
                    stop: -1,
                    aggregate: 'MAX',
...
Flags.validate = async function (payload) {
    ...
    if (payload.type === 'post') {
        ...
        if (
            ...
            reporter.reputation < meta.config['min:rep:flag']
        ) {
...
Flags.update = async function (flagId, uid, changeset) {
    ...
    plugins.hooks.fire('action:flags.update', {
        flagId: flagId,
        changeset: changeset,
        uid: uid,
    });
            ...
            async (pids) => {
                ...
                for (const postObj of postData) {
                    if (parseInt(postObj.flagId, 10)) {
                        ...
                        await Flags.update(postObj.flagId, callerUid, {
                            state: 'resolved',
                        });
                    }
                }
...
Flags.notify = async function (flagObj, uid, notifySelf = false) {
    ...
    if (flagObj.type === 'post') {
        ...
        const targetDisplayname =
            flagObj.target && flagObj.target.user ? flagObj.target.user.displayname : '[[global:guest]]';
        notifObj = await notifications.create({
            type: 'new-user-flag',
