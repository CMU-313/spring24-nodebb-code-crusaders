...
module.exports = {
    ...
    subcommandTerm(cmd) {
        ...
        return (
            chalk[colors[depth].command](cmd._name + (cmd._aliases[0] ? `|${cmd._aliases[0]}` : '')) +
            ...
            .reduce(
                (max, command) => Math.max(
                    ...
                    helper.subcommandTerm(command).length -
                            getControlCharacterSpaces(helper.subcommandTerm(command))
                ),
                0
            );
    },
    ...
    formatHelp(cmd, helper) {
        ...
        function formatItem(term, description) {
            const padding = ' '.repeat(
                termWidth + itemSeparatorWidth - (term.length - getControlCharacterSpaces(term))
            );
            if (description) {
        ...
        const argumentList = helper
            .visibleArguments(cmd)
            .map(argument => formatItem(chalk[colors[depth].arg](argument.term), argument.description));
        ...
        const optionList = helper
            .visibleOptions(cmd)
            .map(option => formatItem(chalk[colors[depth].option](helper.optionTerm(option)), helper.optionDescription(option)));
        ...
        const commandList = helper
            .visibleCommands(cmd)
            .map(cmd => formatItem(helper.subcommandTerm(cmd), helper.subcommandDescription(cmd)));
