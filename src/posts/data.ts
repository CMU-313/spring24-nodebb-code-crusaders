...
export default function (Posts: PostsFunctions) {
    ...
    Posts.getPostData = async function (
        pid: number,
        callback?: (err: Error | null, postData: PostObjectNew | null) => void
    ...
    Posts.setPostField = async function (
        pid: number,
        field: string,
        value: boolean,
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        ...
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
        await plugins.hooks.fire('action:post.setFields', {
            data: { ...data, pid },
        });
    };
}
