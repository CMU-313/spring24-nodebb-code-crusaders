...
module.exports = function (Posts) {
    ...
    Posts.uploads.sync = async function (pid) {
        ...
        await Promise.all([Posts.uploads.associate(pid, add), Posts.uploads.dissociate(pid, remove)]);
    };
    ...
    Posts.uploads.getOrphans = async () => {
        ...
        files = await Promise.all(
            files.map(async filename => ((await Posts.uploads.isOrphan(`files/${filename}`)) ? `files/${filename}` : null))
        );
    ...
    Posts.uploads.dissociate = async function (pid, filePaths) {
        ...
        const promises = [db.sortedSetRemove(`post:${pid}:uploads`, filePaths), db.sortedSetRemoveBulk(bulkRemove)];
        ...
        if (!meta.config.preserveOrphanedUploads) {
            ...
            const uploaderUids = (
                await db.getObjectsFields(deletePaths.map(path => `upload:${md5(path)}`, ['uid']))
            ).map(o => (o ? o.uid || null : null));
            await Promise.all(
                uploaderUids
                    .map((uid, idx) => (uid && isFinite(uid) ? user.deleteUpload(uid, uid, deletePaths[idx]) : null))
                    .filter(Boolean)
            );
