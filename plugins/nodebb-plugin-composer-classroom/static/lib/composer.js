...
define('composer', [
        ...
        if (uuid === undefined) {
            if (title && (selectedPid || toPid)) {
                composer.newReply(
                    ...
                    toPid,
                    title,
                    '[[modules:composer.user_said_in, ' + username + ', ' + link + ']]\n' + text
                );
            } else {
    ...
    async function createNewComposer(post_uuid) {
        ...
        var data = {
            ...
            showHandleInput:
                config.allowGuestHandles && (app.user.uid === 0 || (isEditing && isGuestPost && app.user.isAdmin)),
    ...
    async function post(post_uuid) {
        ...
        var checkTitle =
            (postData.hasOwnProperty('cid') || parseInt(postData.pid, 10)) && postContainer.find('input.title').length;
                ...
                if (data.queued) {
                    ...
                    if (submitHookData.redirect) {
                        ajaxify.go(
                            'topic/' + data.slug,
                            undefined,
                            onComposeRoute || composer.bsEnvironment === 'xs' || composer.bsEnvironment === 'sm'
                        );
                    }
                } else if (action === 'posts.reply') {
