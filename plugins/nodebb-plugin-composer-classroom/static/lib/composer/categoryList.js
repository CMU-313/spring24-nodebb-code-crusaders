...
define('composer/categoryList', ['categorySelector', 'taskbar', 'api'], function (categorySelector, taskbar, api) {
    var categoryList = {};
    ...
    categoryList.init = function (postContainer, postData) {
        ...
        postContainer
            .find('.category-name')
            .translateText(
                selector.selectedCategory ? selector.selectedCategory.name : '[[modules:composer.select_category]]'
            )
            .on('click', function () {
    ...
    function toggleDropDirection(postContainer) {
        postContainer
            ...
            .toggleClass('dropup', postContainer.outerHeight() < $(window).height() / 2);
    }
    ...
    async function changeCategory(postContainer, postData, selectedCategory) {
        ...
        const categoryData = await window
            .fetch(`${config.relative_path}/api/category/${selectedCategory.cid}`)
            .then(r => r.json());
