...
describe('Upload Controllers', () => {
    ...
    describe('regular user uploads', () => {
        ...
        it('should upload an image to a post and then delete the upload', (done) => {
            helpers.uploadFile(
                ...
                (err, res, body) => {
                    ...
                    assert(Array.isArray(body.response.images));
                    assert(body.response.images[0].url);
                    const name = body.response.images[0].url.replace(
        ...
        it('should fail to upload image to post if image dimensions are too big', (done) => {
            helpers.uploadFile(
                `${nconf.get('url')}/api/post/upload`,
                path.join(__dirname, '../test/files/toobig.png'),
                {},
                jar,
                csrf_token,
                (err, res, body) => {
        ...
        it('should fail to upload image to post if image is broken', (done) => {
            helpers.uploadFile(
                `${nconf.get('url')}/api/post/upload`,
                path.join(__dirname, '../test/files/brokenimage.png'),
                {},
                jar,
                csrf_token,
                (err, res, body) => {
    ...
    describe('admin uploads', () => {
        ...
        it('should upload site logo', (done) => {
            helpers.uploadFile(
                `${nconf.get('url')}/api/admin/uploadlogo`,
                path.join(__dirname, '../test/files/test.png'),
                {},
                jar,
                csrf_token,
        ...
        it('should upload default avatar', (done) => {
            helpers.uploadFile(
                `${nconf.get('url')}/api/admin/uploadDefaultAvatar`,
                path.join(__dirname, '../test/files/test.png'),
                {},
                jar,
                csrf_token,
        ...
        it('should upload og image', (done) => {
            helpers.uploadFile(
                `${nconf.get('url')}/api/admin/uploadOgImage`,
                path.join(__dirname, '../test/files/test.png'),
                {},
                jar,
                csrf_token,
        ...
        it('should upload favicon', (done) => {
            helpers.uploadFile(
                ...
                path.join(__dirname, '../test/files/favicon.ico'),
                {},
                jar,
                csrf_token,
                (err, res, body) => {
