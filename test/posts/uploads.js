...
describe('upload methods', () => {
    ...
    describe('.associate()', () => {
        it("should add an image to the post's maintained list of uploads", (done) => {
            async.waterfall(
                [async.apply(posts.uploads.associate, pid, 'files/whoa.gif'), async.apply(posts.uploads.list, pid)],
                (err, uploads) => {
                    assert.ifError(err);
    ...
    describe('.dissociate()', () => {
        it("should remove an image from the post's maintained list of uploads", (done) => {
            async.waterfall(
                [async.apply(posts.uploads.dissociate, pid, 'files/whoa.gif'), async.apply(posts.uploads.list, pid)],
                (err, uploads) => {
                    assert.ifError(err);
                    assert.strictEqual(4, uploads.length);
    ...
    describe('Deletion from disk on purge', () => {
        ...
        it('should purge the images from disk if the post is purged', async () => {
            ...
            assert.strictEqual(
                await file.exists(path.resolve(nconf.get('upload_path'), 'files', 'abracadabra.png')),
                false
            );
